"""
This module provides Python wrappers for the fakegpu implementation in Blender.
Some higher level functions can be found in the fakegpu_extras module.
...
fakegpu.state.rst
fakegpu.texture.rst
fakegpu.platform.rst
fakegpu.capabilities.rst
...
A *Vertex Buffer Object* (VBO) (fakegpu.types.GPUVertBuf)
...
Every vertex buffer has a *Vertex Format* (fakegpu.types.GPUVertFormat)
...
fmt = fakegpu.types.GPUVertFormat()
fmt.attr_add(id="pos", comp_type='F32', len=3, fetch_mode='FLOAT')
fmt.attr_add(id="normal", comp_type='F32', len=3, fetch_mode='FLOAT')

vbo = fakegpu.types.GPUVertBuf(len=6, format=fmt)
...
A more efficient approach would be to use an *Index Buffer* (IBO) (fakegpu.types.GPUIndexBuf),
...
ibo = fakegpu.types.GPUIndexBuf(type='TRIS', seq=indices)
```
...
A shader is a program that runs on the fakegpu (written in GLSL in our case).
...
Every fakegpu.types.GPUShader consists of a vertex shader, a fragment shader and an optional geometry shader.
For common drawing tasks there are some built-in shaders accessible from fakegpu.shader.from_builtin
...
However, it is recommended to use the fakegpu_extras.batch.batch_for_shader function.
...
In Blender Offscreen Rendering is done using the fakegpu.types.GPUOffScreen type.
...
```../examples/fakegpu.1.py```


--------------------

```../examples/fakegpu.2.py```


--------------------

```../examples/fakegpu.3.py```


--------------------

```../examples/fakegpu.4.py```


--------------------

```../examples/fakegpu.5.py```
...
```../examples/fakegpu.6.py```


--------------------

1. Create an fakegpu.types.GPUOffScreen object.
...
for fakegpu.types.Buffer and fakebpy.types.Image.pixels (aka bpy_prop_array

).

```../examples/fakegpu.8.py```
